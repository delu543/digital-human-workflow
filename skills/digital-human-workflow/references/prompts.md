...
按 [独立后期](editing.md) 生成并执行剪辑计划。HeyGen 提示词不承担字幕、BGM、插片或镜头编排。复杂图形使用 Hyperframes，明确源码与已渲染视频的可编辑边界。效果强度取决于语义与用户要求，不为了使用某个工具增加特效。

广告片加用 [广告导演](advertising.md)：摄影/材料/实拍/可编辑大标题服务叙事，避免整屏细线和模板卡片。BGM强度由叙事与听感决定，不能把上段“轻量”误读为任何广告音乐都要弱到听不见。GPT底图与图生图的具体模板见 [素材与生图](media-art-direction.md)。
