...
Codex 负责理解文案、分镜与执行，MiniMax 负责用户自己的声音，HeyGen 负责干净数字人口播。字幕从实际声音对齐，后期可直接用 Hyperframes，也可选择 Mac 剪映工程交付，剪映不是必需依赖。默认中文、9:16、1080×1920，目标约五分钟；实际时长由配音决定。

这是可安装的通用工作流与 Codex Skill，不包含作者账号、录音、私人文案、形象、密钥或个人配置。每位用户在自己的电脑保存自己的素材与账户设置。

当前版本 **v0.3.0**：新增独立剪辑时间轴、不可覆盖的修订、句段剪切/重排/同步变速、视频插片、独立字幕/标题/混音、基础镜头关键帧，以及可选的实验性 Mac 剪映草稿。保留此前的声音与真人感验收。详见 [更新记录](CHANGELOG.md)、[剪辑流程](skills/digital-human-workflow/references/editing.md) 与 [真人感流程](skills/digital-human-workflow/references/realism.md)。
...
或要求“同时给我 Mac 剪映可编辑工程”。Codex 编写计划并执行本地剪辑，用户无需填写 JSON。只修改后期时复用已经生成的声像，不自动重新调用付费模型。训练视频用于创建本人数字人，不以真人拍摄出镜替代用户要求的数字人。
